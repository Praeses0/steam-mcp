  ...
  server.tool(
    ...
    'Add a non-Steam game shortcut',
    {
      ...
      start_dir: z
        ...
        .optional()
        .describe(
          "Working directory for the executable (defaults to the exe's parent directory)",
        ),
      launch_options: z
        .string()
        .optional()
      ...
      tags: z
        .array(z.string())
        .optional()
        .describe('Tags / categories for the shortcut'),
    },
    async ({ name, exe, start_dir, launch_options, tags }) => {
      try {
        const shortcuts = readShortcuts();
        const startDir = start_dir ?? path.dirname(exe);
        const appid = generateShortcutId(exe, name);
  ...
  server.tool(
    ...
    'Remove a non-Steam game shortcut',
    {
      ...
      name: z
        .string()
        .optional()
        .describe('The display name of the shortcut to remove (used if appid is not provided)'),
    },
    async ({ appid, name }) => {
      try {
  ...
  server.tool(
    ...
    'Import Lutris games as non-Steam shortcuts',
    {
      game_ids: z
        ...
        .optional()
        .describe(
          'Specific Lutris game IDs to import. If omitted, all Lutris games are imported.',
        ),
    },
    async ({ game_ids }) => {
      try {
        ...
        for (const game of lutrisGames) {
          ...
          const exe = 'lutris';
          const launchOptions = `lutris:rungameid/${game.id}`;
          const appid = generateShortcutId(exe, game.name);
